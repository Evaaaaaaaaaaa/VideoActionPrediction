...
def lstm_model(max_action_len):
    ...
    action = LSTM(128)(i1)
    goal = Dense(10, activation="softmax")(action)
    goal_repeat = RepeatVector(max_action_len)(goal)

    # 3 layers
    action = LSTM(128, return_sequences=True)(i1)
    action = LSTM(64, return_sequences=True)(action)
    action = LSTM(32, return_sequences=True)(action)

    # late fusion concat and predict action
    i2 = tf.concat([action, goal_repeat], -1)
    action = LSTM(128)(i2)
    action = Dense(47, activation="softmax")(action)
    model = Model(inputs=i1, outputs=[action, goal])
...
def tcn_model(max_action_len):
    ...
    # 3 layers
    action = TCN(128, return_sequences=True)(i1)
    action = TCN(64, return_sequences=True)(action)
    action = TCN(32, return_sequences=True)(action)
...
def transformer_model(max_action_len):
    ...
    i = transformer_block1(inputs)
    i = transformer_block1(i)
    i = transformer_block1(i)

    i = tf.concat([i, o1_repeat], -1)
    transformer_block2 = TransformerBlock(57, 1, ff_dim)
    i = transformer_block2(i)
    i = layers.Flatten()(i)
    action = layers.Dense(47, activation="softmax")(i)
    model = keras.Model(inputs=inputs, outputs=[action, o1])
...
def train_model(trainX, trainY, model, sample_weight):
    ...
    model.fit(np.array(trainX), np.array(trainY[0]), epochs=20, batch_size=128, shuffle = True)
...
def cross_validation(all_split_x, all_split_y_action, all_split_y_goal, model):
    ...
    for i in range(4):
        ...
        m = model
        m.fit(np.array(train_all_fold_x[i]), [np.array(train_all_fold_y_action[i]), np.array(train_all_fold_y_goal[i])], epochs=20, batch_size=128, shuffle=True)
