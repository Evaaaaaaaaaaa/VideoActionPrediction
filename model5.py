...
def add_activity(wo_activity_input, encoded_goal_y):
    ...
    for filename, actions in wo_activity_input.items():
        ...
        for i in range(len(actions)):
            if actions[i] == 47 * [0]:
                w_activity_input[filename].append(actions[i] + 10 * [0])
            else:
...
def cross_validation(wo_input_dict, w_input_dict, goal_y, action_y, wo_model, w_model, max_timesteps, is_goal):
    file_count = 0
    s1_x1, s2_x1, s3_x1, s4_x1 = [], [], [], []
    s1_x2, s2_x2, s3_x2, s4_x2 = [], [], [], []
    ...
    for i in range(4):
        ...
        for j in range(4):
            if i != j:
                trainX_w += copy.deepcopy(splits_x1[j])
                trainX_wo += copy.deepcopy(splits_x2[j])
                trainY_goal += copy.deepcopy(splits_goal_y[j])
                trainY_action += copy.deepcopy(splits_action_y[j])

        testX = copy.deepcopy(splits_x2[i])
        testY_goal = copy.deepcopy(splits_goal_y[i])
        testY_action = copy.deepcopy(splits_action_y[i])
        sample_weight = get_sample_weight(trainX_wo)

        # train with input frame + true goal
        # train_model(trainX_w, trainY_action, w_model, sample_weight)
        # x: input frame, y: goal
        train_model(trainX_wo, trainY_goal, wo_model, sample_weight)
        temp_goal_acc, predicted_goal = evaluation(testX, testY_goal, wo_model, max_timesteps, 1)
        ...
        for j in range(len(testX)):
            ...
            for k in range(len(testX[j])):  # todo if frame is padding, add [0]*10
                if testX[j][k] == 47 * [0]:
                    each_video.append(testX[j][k] + 10 * [0])
                else:
                    each_video.append(testX[j][k] + predicted_goal_list[i][j].tolist())
            each_split.append(each_video)
        temp_action_acc, predicted_action = evaluation(each_split, testY_action, w_model, max_timesteps, 0)
...
def run_model():
    ...
    for proportion in input_proportion:
        ...
        train_y.extend(encoded_action_y)
    ...
    for proportion in input_proportion:
        ...
        # transformer
        wo_transformer_goal_acc,  wo_transformer_action_acc = cross_validation(wo_activity_input, w_activity_input,encoded_goal_y, encoded_action_y,
